# Ezaz Risan — Niche Site Portfolio
...
## 2b. Add your photo to the contact section

The contact section (bottom of every page) has a photo placeholder next to
the "Have a niche in mind?" text — currently a dashed box with "ER" in it.

1. Save a photo of yourself into the `assets/` folder as, e.g., `assets/profile.jpg`
   (a square or 4:5 portrait crop works best).
2. In `index.html`, find the `cta-photo-frame` block and replace:
   ```html
   <div class="cta-photo-placeholder">ER</div>
   <span class="cta-photo-tag">Add your photo — see README</span>
   ```
   with:
   ```html
   <img src="assets/profile.jpg" alt="Ezaz Risan">
   ```
3. Do the same in each `projects/*/index.html` file, using `../../assets/profile.jpg`
   as the path instead (since case study pages sit one folder deeper).
